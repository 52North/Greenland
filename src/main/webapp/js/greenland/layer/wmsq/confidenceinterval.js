...
OpenLayers.Layer.WMSQ.ConfidenceInterval = OpenLayers.Class(OpenLayers.Layer.WMSQ.Visualization,
		{
			...
			initialize : function(options) {
				...
				options.styler.bounds.fixedMinValue = options.layerOptions[0].min;
				options.styler.bounds.fixedMaxValue = options.layerOptions[0].max;
			...
			fillCanvas : function(canvas, merger, ctx, tile) {
				...
				for ( var y = 0; y < height; y += sy)
					for ( var x = 0; x < w; x += sx) {
						...
						value = reqLayers.getConfidenceInterval.call(this, merger, x, y, sx, sy, distribution,
								p);
						if (value == null) {
							// No value
							continue;
